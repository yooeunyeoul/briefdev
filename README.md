...
src/
  ...
  components/card/
    ...
    CardItem.tsx          # 카드 비주얼
    ShareBar.tsx          # Web Share API + 클립보드 fallback
  lib/
...
### Claude Code가 어떻게 도왔나 (메커니즘)

정확한 시간 단축 *배수*는 측정하지 않았지만, 다음 3가지가 *체감상* 가장 큰 레버리지:

1. **`CLAUDE.md` 일관성** — 도메인 용어 / 컨벤션 / 금지 패턴이 매 세션에 자동 주입되어 *"Next.js 16 새 API 어떻게 쓰지?"* 같은 검색을 코드 안에서 즉시 해결
2. **프롬프트 버전 관리 (`prompts/curate-vN.md`)** — LLM 큐레이션 자체를 *코드처럼 diff* 로 진화시켜 *6번 가설→실측→갱신* 사이클을 빠르게 반복
3. **Atomic 커밋 + PDCA 사이클** — 작은 변경 → 즉시 빌드 검증 → 커밋 → 다음 작업 흐름으로 *되돌리기 쉬운 작업 단위* 유지

### AI가 못한 것 / 사람이 한 것

- **사용자 신뢰의 임계값 설정** — "5장 강제 vs 2장이라도 정직?" 판단은 사람이 결정
- **페르소나 정의** — "한국 개발자 = 누구?"의 디테일 (서브 페르소나 포함 폭 결정)
- **whyMatters 문체 톤** — "~을 하는 개발자에게 — 가치" 형태 발견까지 v1→v5 여러 번 시행착오
...
이 프로젝트의 핵심 가치는 **6번에 걸쳐 큐레이션 품질을 끌어올린 사이클**입니다.
...
v6: + 한국 개발자 커뮤니티 3개 (GeekNews + 토스 + 카카오) + Google AI Blog
   ✅ 페르소나 매칭 보강 — 한국 출처 추가로 'kr' 카테고리 진짜 활성화
   ✅ Atom 파서 single-quote 네임스페이스 지원 (GeekNews 호환)
   ✅ 풀 크기 확장 (총 9개 소스)
   ⚠️ 한국 출처여도 일반 DB/인프라 글은 8점 임계 통과 X (출처 가중치 ≠ AI 관련성)
```
...
### 실측 결과 (v6, 매일 변동)
- 풀 크기: **약 40~50개** (HF/Simon/Verge/Google AI/GeekNews/토스/카카오 각 최대 5 + HN top 12, paywall 자동 제외, URL 중복 제거 후)
- 본문 추출 성공률: **~50% 내외** (한국 블로그 일부는 Jina 봇 차단)
- 카드 출력: **1~5장** (8점 임계 통과 수에 따라 가변)
- 출처 분산: **다수 소스에 분포** (한 소스 5장 독점 방지)
- LLM 호출: **1회/일**
...
| Phase | 수익 모델 | 가설 |
|-------|---------|------|
| 1 (지금~6개월) | **무료** | 리텐션 + 사용자 수 검증 우선 (가격 마찰 0) — News 앱 평균 anchor 도달 (D1 ≥ 26.67% / D7 ≥ 15.83% / D30 ≥ 7.55%, OneSignal 검증) |
| 2 (사용자 임계 후) | **Google AdSense** | 1인 운영 가능, 트래픽 → 즉시 매출 검증. 단가 한계 베이스라인 측정 후 판단. |
| 3 (AdSense 한계 후) | **직접 스폰서십 / B2B SaaS** ⭐ | AI 도구 회사 한국 마케팅 채널 / 팀 도메인 맞춤 다이제스트 — 데이터 보고 베팅 |
...
가설("'~을 하는 개발자에게' whyMatters가 News 앱 평균 리텐션 (OneSignal 검증) 도달을 만든다")을 검증할 수 있도록:
...
- RSS 7개 — Hugging Face Blog / Simon Willison / The Verge AI / Google AI Blog / GeekNews / 토스 기술 블로그 / 카카오 기술 블로그 (모두 무료)
